...
def check_permission(user, method, permission_to):
    ...
    required_permissions = 'view_'+permission_to
    if method == 'POST':
        required_permissions = 'add_'+permission_to
    elif method == 'PUT':
        required_permissions = 'change_'+permission_to
    elif method == 'DELETE':
        required_permissions = 'delete_'+permission_to

    groups = User_Groups.objects.values('group').filter(user_id=user.id).all()
    for group in groups:
        permissions = Group_Permissions.objects.values('permission_id').filter(group_id=group['group_id']).all()

        for permission in permissions:
            if Permission.objects.filter(id=permission['permission_id'], codename=required_permissions).exists():
                return True
